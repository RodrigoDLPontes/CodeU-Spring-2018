...
<head>
  ...
  <script type="text/javascript">
    ...
    function drawFirstChart() {
      ...
      <% List<Statistic> statistics = instance.loadStatistics(Type.USER_STORE_ADD_USER_TIME); %>
      var table = new Array();
      ...
      // Set chart options
      var options = {'title':'User Store addUser() Elapsed Time',
                     'width':800,
                     'height':600,
      ...
      var chart = new google.visualization.LineChart(document.getElementById('add_user_chart_div'));
      chart.draw(data, options);
    }

    function drawSecondChart() {

        <% statistics = instance.loadStatistics(Type.USER_STORE_GET_USER_ID_COMPS); %>
        var table = new Array();
        <% for (int i = 0; i < statistics.size(); i++) {
             String date = statistics.get(i).getCreationTimeString();
             long value = statistics.get(i).getValue();
        ...
        var options = {'title':'User Store getUser(UUID id) Number of Comparisons',
                       'width':800,
                       'height':600,
                       'backgroundColor':'#f6f6f6',
                       'colors':['green'],
                       'hAxis':{'title':'Creation time (date)'},
                       'vAxis':{'title':'Number of comparisons'},
                       'legend':{'position':'none'},
                       'chartArea':{'left':'15%','top':'15%','width':'70%','height':'60%'},
                       'animation':{'startup':true, 'duration':500, 'easing':'in'}
        };
    ...
    function drawThirdChart() {
        ...
        var options = {'title':'User Store getUser(String username) Number of Comparisons',
                       'width':800,
                       'height':600,
                       'backgroundColor':'#f6f6f6',
                       'colors':['green'],
    ...
    function drawFourthChart() {
        ...
        var options = {'title':'User Store isUserRegistered() Number of Comparisons',
                       'width':800,
                       'height':600,
                       'backgroundColor':'#f6f6f6',
                       'colors':['green'],
    ...
    function drawFifthChart() {
        ...
        var options = {'title':'User Store updateUser() Number of Comparisons',
                       'width':800,
                       'height':600,
                       'backgroundColor':'#f6f6f6',
                       'colors':['green'],
...
<body>
  ...
  <div id="container">
    ...
    <div id="add_user_chart_div"></div>
    <br>
    <div id="get_user_id_chart_div"></div>
    <br>
    <div id="get_user_username_chart_div"></div>
    <br>
    <div id="is_user_registered_chart_div"></div>
    <br>
    <div id="update_user_chart_div"></div>
    <br>
  </div>
